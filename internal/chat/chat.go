// Package chat extract nick, msg and state from raw message
package chat
...
func ExtractMessage(chat *ChatMessage, rawMsg []byte) error {
	...
	if err := extractBadges(rawMsg, chat.State); err != nil {
...
// extractBadges find badges in raw message by 'badges=' pattern
// and set state to subscriber, leadMod or moderator
func extractBadges(rawMsg []byte, state *int) error {
	...
	rangeByByte(badgesContent, ',', func(splice []byte) {
		if bytes.Contains(splice, []byte("subscriber")) {
			*state = subscriber
		} else if bytes.Contains(splice, []byte("lead_moderator")) {
			*state = leadMod
		} else if bytes.Contains(splice, []byte("moderator")) {
			*state = moderator
		}
	})
